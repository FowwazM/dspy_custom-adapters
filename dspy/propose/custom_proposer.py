...
def generate_instruction_class(
    ...
    class GenerateSingleModuleInstruction(dspy.Signature):
        ...
        if program_aware:
            ...
            if use_custom_task:
                custom_task = dspy.InputField(
                    desc="Your specific guidelines for the proposed instruction. IMPORTANT: These guidelines MUST be followed UNDER ALL CIRCUMSTANCES when generating a proposed instruction.", prefix="GUIDELINES:"
                )
...
class GenerateModuleInstruction(dspy.Module):
    def __init__(
        ...
        if custom_task is not None:
            self.use_custom_task = True
        else:
            self.use_custom_task = False
        ...
        self.generate_module_instruction = generate_instruction_class(
            ...
            use_tip=use_tip,
            use_custom_task=self.use_custom_task,
        )
        ...
        for pred_i, predictor in enumerate(program.predictors()):
            for demo_set_i in range(num_demos):
                ...
                if self.set_tip_randomly:
                    if self.verbose:
                        print("Using a randomly generated configuration for our custom proposer.")
